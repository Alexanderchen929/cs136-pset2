...
class AccStd(Peer):
    ...
    def uploads(self, requests, peers, history):
        ...
        if round == 0:
            ...
            # sort by the most and select only the top 3
            peer_ranking = dict(sorted(download_totals.items(),key=lambda x: x[1], reverse=True))
            peer_ranking_list = list(peer_ranking.keys())
            chosen_peers = peer_ranking_list[:3]
            ...
            chosen = list(rp_set.union(cp_set))
